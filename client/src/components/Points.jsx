...
const Edumpers = () => {
    ...
    const verifyCoupon = async () => {
        try {
            const response = await axios.post('http://localhost:5000/api/verify-coupon', { couponCode });
            setMessage(response.data.message);
        } catch (error) {
            console.error('Full error object:', error);
            console.error('Error response:', error.response);
            setMessage(error.response?.data?.message || "Error verifying coupon. Please try again.");
        }
    }
                ...
                <div className="flex flex-row w-1/3 justify-around items-start">
                    <div className="mr-20">
                        ...
                        <div className="flex justify-start items-start flex-col">
                            ...
                            <input onChange={(e) => setWeight(e.target.value)} type="number" value={weight} className="w-64 bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" />
                        </div>
                        <div className="mt-5">
                            <button onClick={calculatePoints} className="w-64 flex justify-center mx-auto text-white bg-green-500 border-0 py-2 focus:outline-none hover:bg-green-600 rounded text-lg">Calculate Points</button>
                        </div>
                        {/* <div className="mt-5">
                            <button onClick={handleSubmit} className="w-64 flex justify-center mx-auto text-white bg-blue-500 border-0 py-2 focus:outline-none hover:bg-blue-600 rounded text-lg">Submit</button>
                        </div> */}
                        </div>
                    <div className="flex justify-center items-center w-64 mr-20 mx-auto">
                        <div className="flex flex-wrap">
                            ...
                            <div className="mt-5 w-64">
                                <button onClick={verifyCoupon} className="flex mx-auto justify-center text-white bg-green-500 border-0 py-2 px-8 focus:outline-none hover:bg-green-600 rounded text-lg">Verify Coupon</button>
                            </div>
                        </div>
                    </div>
